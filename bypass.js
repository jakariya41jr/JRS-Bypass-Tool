...
(function () {
  ...
  let CONFIG = {
    ...
    fallbackRedirectUrl: "https://jakariya41jr.xyz/",
  ...
  const DEFAULT_USER_DATA = {
    id: 0,
    name: "JRS",
    password: "jrs41",          // Only password
    tgChannel: "0",
    banned: 0,
    creator: "Md Jakariya Hasan",
    chatId: "",
    createdAt: ""
  };
  ...
  let autoInitTimeout = null, isRedirecting = false;
  let exploitProgressActive = false;
  let exploitProgressRAF = null;
  let logTimers = [], redirectUrlCache = null;
  ...
  DBG.log('INIT', 'TOTPGenerator ready');
  ...
  async function fetchRedirectUrlFromAPI(type, attempt = 1) {
    ...
    try {
      ...
      if (attempt > 1) queueLog('🔄', `ATTEMPT ${attempt} OF ${maxRetries}`, '#ffa500', 'log-highlight');
  ...
  function needPassword() { return true; }
  ...
  function nextTrackManual() {
    if (!shouldPlayMusic()) { showToast("📵 Music blocked on mobile data"); return; }
    nextTrackAuto();
  ...
  function setupMusicToggle(btnId) {
    ...
    musicBtn.addEventListener("click", () => {
      ...
      if (!audioPlayer) { initAudioConditionally(); updateMusicBtnAppearance(); return; }
  ...
  function renderInitPanel() {
    ...
    const { wrapper, focusGlow1, focusGlow2 } = createWrapper(`
      ...
      <div class="nb-footer">© Md Jakariya Hasan | ${APP_FULL_NAME} | 📳 Shake to change track 🎵</div>
    `, "overflow-visible");
  ...
  async function handleTargetSelect(target, targetName, apiType, moduleType, selectionOverlay, authOverlay) {
    ...
    const btns = ["target-aincrad","target-aincrad-proxy","target-vipteam","target-powercheats","target-universal-vplink"];
    ...
    if (authOverlay) { authOverlay.style.transition = "opacity 0.3s"; authOverlay.style.opacity = "0"; }
  ...
  function extractVplinkFromPage() {
    try {
      ...
      const bodyMatch = bodyText.match(/https?:\/\/vplink\.in\/[^\s"'<>]+/);
      if (bodyMatch) return bodyMatch[0].replace(/[.,;:'")\]}]+$/, '');
  ...
  function extractVpKey(vplinkUrl) {
    try {
      let cleanUrl = vplinkUrl.trim().split('?')[0].split('#')[0];
  ...
  function extractVplinkFromPagePowerCheats() {
    try {
      ...
      if (currentURL.includes('vplink.in')) {
        ...
        return match ? match[0].replace(/[.,;:'")\]}]+$/, '') : currentURL;
      }
      ...
      if (htmlMatch) return htmlMatch[0].replace(/[.,;:'")\]}]+$/, '');
      return null;
    } catch {
      return null;
    }
  ...
  function renderUniversalVplinkPanel(apiType) {
    ...
    async function fetchUniversalVplinkRedirectUrl(type, vpKey, attempt = 1) {
      const maxRetries = 3;
      try {
        const pin = await totpGenerator.generate();
        currentPinCache = pin;
    ...
    urlInput.addEventListener("input", function() {
      ...
      if (rawUrl.length > 0 && rawUrl.toLowerCase().includes('vplink.in')) {
